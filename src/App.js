        ...
        {step === 1 && (
          <div className="space-y-8">
            <div className="text-center mb-10">
              ...
              <h1 className="text-3xl font-black tracking-tight text-white mb-1">SMART-EDU</h1>
        ...
        <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4">
          <div className="w-full max-w-md bg-slate-900 border border-slate-800 rounded-3xl p-6">
            ...
            <div className="mt-5 flex justify-end gap-3">
              <button onClick={() => setShowForgotModal(false)} className="px-4 py-2.5 rounded-xl bg-slate-800 text-slate-300 text-sm font-bold">Close</button>
        ...
        <table className="w-full text-center border-collapse min-w-[600px]">
          ...
          <tbody className="text-[13px]">
            {TIMETABLE_SLOTS.map((slot, idx) => {
              ...
              return (
                <tr key={idx} className={`border-t border-slate-800 ${slot.isBreak ? 'bg-amber-500/5' : ''} ${isCurrent ? 'bg-indigo-500/5' : ''}`}>
                  <td className="py-5 border-r border-slate-800 bg-slate-900/50">
                    <p className={`font-bold ${isCurrent ? 'text-indigo-300' : 'text-slate-300'}`}>{slot.time}</p>
...
const QRGenerator = ({ subject = 'General', durationMinutes = 15, facultyId = 'faculty' }) => {
  ...
  return (
    <div className="flex flex-col items-center gap-4 bg-white p-6 rounded-2xl">
      ...
      <p className="text-[11px] text-slate-500 uppercase font-black tracking-widest">Scan to mark attendance</p>
      <p className="text-[10px] text-slate-400">Subject: <strong>{subject}</strong> · Expires: {expiresAt.toLocaleTimeString()}</p>
    </div>
  );
};
...
const AIChatFAB = () => {
  ...
  const send = async () => {
    ...
    try {
      ...
      setMsgs(prev => [...prev, { role: 'assistant', content: '⚠️ EduAI is temporarily unavailable. Please ensure the backend is running with a valid GEMINI_API_KEY.' }]);
        ...
        <div className="mb-4 w-80 lg:w-96 bg-slate-900 border border-slate-700 rounded-3xl shadow-2xl flex flex-col overflow-hidden" style={{ height: 480 }}>
          <div className="p-4 border-b border-slate-800 flex items-center justify-between bg-indigo-600/10">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm font-bold text-white">EduAI Study Buddy</p>
            ...
            {messages.map((m, i) => (
              <div key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[80%] px-4 py-3 rounded-2xl text-sm leading-relaxed whitespace-pre-wrap
                  ${m.role === 'user' ? 'bg-indigo-600 text-white rounded-br-sm' : 'bg-slate-800 text-slate-200 rounded-bl-sm'}`}>
                  {m.content}
                </div>
              </div>
            ))}
          ...
          <div className="flex-1 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-slate-800/30 p-4 rounded-2xl border border-slate-800/50">
                <p className="text-[9px] text-slate-500 uppercase font-black tracking-widest mb-1">Semester CGPA</p>
...
const ResultsPage = () => {
  ...
  return (
    <div className="max-w-5xl mx-auto w-full space-y-6 lg:space-y-8">
      <div className="flex justify-end"><MarksheetPDF student={marksheetStudent} /></div>

      {/* Overall CGPA Banner */}
      <div className="bg-gradient-to-r from-indigo-600/20 to-cyan-600/20 border border-indigo-500/30 rounded-2xl p-5 flex items-center justify-between">
        <div>
          <p className="text-[10px] text-slate-400 uppercase font-black tracking-widest mb-1">Overall CGPA (10.0 Scale)</p>
          <p className="text-4xl font-black text-white">
            {calcCGPA10(studentData.results.flatMap(r => r.subjects))}
            <span className="text-lg text-slate-400 font-medium"> / 10.0</span>
          </p>
        </div>
        <div className="text-right">
          <p className="text-[10px] text-slate-400 uppercase font-black tracking-widest mb-1">Grade</p>
          <p className="text-2xl font-black text-indigo-400">{gradePointToLetter(parseFloat(calcCGPA10(studentData.results.flatMap(r => r.subjects))))}</p>
        </div>
      </div>
        ...
        <div key={idx} className="bg-slate-900 border border-slate-800 rounded-[2rem] overflow-hidden shadow-xl">
          <div className="p-6 lg:p-8 border-b border-slate-800 bg-slate-800/20 flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              ...
              <p className="text-xs text-slate-500">Academic Performance Report · 10.0 Scale</p>
            </div>
            <div className="flex items-center gap-6">
              <div>
                <p className="text-[10px] text-slate-500 uppercase font-black tracking-widest mb-1">SGPA</p>
                <p className="text-3xl font-black text-indigo-400">{calcCGPA10(result.subjects)}</p>
              </div>
            ...
            <table className="w-full text-left border-collapse min-w-[500px]">
              <thead className="bg-slate-800/50 text-[10px] uppercase font-black tracking-widest text-slate-500 border-b border-slate-800">
                <tr>
                  ...
                  <th className="px-6 lg:px-8 py-4 text-center">Grade Point</th>
                  <th className="px-6 lg:px-8 py-4 text-center">Grade</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-800/60">
                {result.subjects.map((s, si) => {
                  const gp = s.gradePoint ?? marksToGradePoint(s.marks || 0);
                  ...
                  return (
                    <tr key={si} className="hover:bg-slate-800/20 transition-colors">
                      ...
                      <td className="px-6 lg:px-8 py-4 text-center font-black text-indigo-400">{gp}</td>
                      <td className="px-6 lg:px-8 py-4 text-center">
                        <span className={`px-3 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-wider ${letter.includes('A') || letter === 'O' ? 'bg-emerald-500/10 text-emerald-500 border border-emerald-500/20' : 'bg-indigo-500/10 text-indigo-500 border border-indigo-500/20'}`}>
                          {letter}
                        </span>
                      </td>
                    </tr>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-[2rem] overflow-hidden shadow-xl">
        ...
        <div className="p-4 space-y-4">
          {studentData.assessments.map((a) => (
            <div key={a.id} className="bg-slate-800/40 border border-slate-800 rounded-2xl p-5 hover:bg-slate-800/60 transition-colors">
...
const SyllabusPage = () => {
  ...
  const simulateUpload = (code) => {
    setUploadingCode(code);
    setTimeout(() => setUploadingCode(null), 2000);
  };

  return (
    <div className="w-full">
      {isFaculty && (
        <div className="mb-6 flex items-center gap-3 bg-indigo-500/10 border border-indigo-500/30 rounded-2xl px-5 py-3">
          <Settings size={16} className="text-indigo-400" />
          <p className="text-sm text-indigo-300 font-bold">Faculty Mode — Click ✏️ to edit subject details and upload materials.</p>
        </div>
      )}
          ...
          <div key={sub.code} className="bg-slate-900 border border-slate-800 p-6 lg:p-8 rounded-[2rem] hover:border-indigo-500/50 transition-all group shadow-xl">
            ...
            {isFaculty && editing === sub.code ? (
              <textarea
                ...
                className="w-full bg-slate-800 border border-slate-700 rounded-xl p-3 text-sm text-slate-300 outline-none focus:border-indigo-500 resize-none h-20 mb-4"
              />
            ) : (
              <p className="text-sm text-slate-500 mb-8 leading-relaxed h-12 overflow-hidden">{sub.description}</p>
            )}

            <div className={`flex items-center justify-between ${isFaculty && editing === sub.code ? '' : 'pt-6 border-t border-slate-800/60'}`}>
              <span className="text-[10px] text-slate-600 uppercase font-black tracking-widest">{sub.modules} Modules</span>
              {isFaculty ? (
                <div className="flex gap-2">
                  <button
                    onClick={() => simulateUpload(sub.code)}
                    className="flex items-center gap-1 text-indigo-400 text-xs font-black uppercase tracking-widest bg-indigo-500/10 px-3 py-2 rounded-xl border border-indigo-500/20 hover:bg-indigo-500 hover:text-white transition-all">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-[2.5rem] overflow-hidden shadow-2xl">
        <div className="p-6 border-b border-slate-800 flex flex-col md:flex-row md:items-center justify-between gap-4 bg-slate-800/20">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className="bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-black uppercase tracking-widest px-6 py-3 rounded-2xl transition-all shadow-lg shadow-indigo-600/10">
              Submit Roll Call
            </button>
          </div>
        </div>
        <div className="p-4 space-y-3">
          {students.map(({ id, name }) => {
            ...
            return (
              <div key={id} className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-4 rounded-[1.5rem] bg-slate-800/20 hover:bg-slate-800/40 transition-colors border border-transparent hover:border-slate-700">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-slate-500 font-bold uppercase tracking-widest mt-0.5">{id}</p>
                  </div>
                </div>
                <div className="flex gap-2">
                  ...
                  <button
                    ...
                    className={`flex-1 sm:flex-none px-5 py-2.5 rounded-xl text-[11px] font-black uppercase tracking-widest border transition-all flex items-center gap-1.5 justify-center
                      ${status === 'present'
                        ...
                        : 'bg-slate-950 text-slate-500 border-slate-800 hover:border-emerald-500/50 hover:text-emerald-400'}`}>
                  ...
                  <button
                    ...
                    className={`flex-1 sm:flex-none px-5 py-2.5 rounded-xl text-[11px] font-black uppercase tracking-widest border transition-all flex items-center gap-1.5 justify-center
                      ${status === 'absent'
                        ? 'bg-red-500 text-white border-red-400/30 shadow-lg shadow-red-500/20'
                        : 'bg-slate-950 text-slate-500 border-slate-800 hover:border-red-500/50 hover:text-red-400'}`}>
          ...
          <div className="flex flex-col items-end gap-2">
            <div className="flex flex-wrap gap-3 justify-end">
              <button onClick={() => exportMarks(marks)}
                className="bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-black uppercase tracking-widest px-5 py-3 rounded-2xl flex items-center gap-2">
              ...
              <button onClick={handleSaveAll} disabled={saving}
                className="bg-indigo-600 hover:bg-indigo-500 disabled:opacity-60 text-white text-xs font-black uppercase tracking-widest px-5 py-3 rounded-2xl flex items-center gap-2">
      ...
      <div className="p-6 lg:p-8 border-b border-slate-800 flex flex-col md:flex-row md:items-center justify-between gap-4 bg-slate-800/20">
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          <button onClick={() => exportStudentList(facultyData.managedStudents)}
            className="bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-black uppercase tracking-widest px-5 py-3 rounded-2xl flex items-center gap-2 whitespace-nowrap">
            <Download size={16} /> Export Excel
          </button>
        </div>
      </div>
            ...
            {filtered.length > 0 ? filtered.map(s => (
              <tr key={s.id} className="hover:bg-slate-800/20 transition-colors">
                ...
                <td className="px-6 lg:px-8 py-5 text-center">
                  <button onClick={() => navigate(`/student/${s.id}`)} className="text-slate-500 hover:text-indigo-400 transition-colors p-2 hover:bg-slate-800 rounded-lg">
                    <Eye size={18} />
                  </button>
                </td>
              </tr>
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-8 w-full max-w-5xl mx-auto items-stretch">
      ...
      <div
        ...
        onDrop={handleDrop}
        className={`bg-slate-900 border-2 border-dashed rounded-[2.5rem] p-10 flex flex-col items-center justify-center cursor-pointer transition-all shadow-xl
          ${isDragging ? 'border-indigo-500 bg-indigo-500/5 scale-105' : 'border-slate-800 hover:border-indigo-500/50'}`}
        onClick={() => fileInputRef.current?.click()}>
        <div className={`w-20 h-20 rounded-3xl flex items-center justify-center mb-6 transition-all duration-300 ${isDragging ? 'bg-indigo-500/20 text-indigo-400' : 'bg-slate-800 text-slate-500'}`}>
          {isDragging ? <Paperclip size={28} /> : <Upload size={28} />}
        </div>
        <h4 className="text-xl font-bold text-white text-center">
          {isDragging ? 'Drop files here!' : 'Upload Materials'}
        </h4>
        <p className="text-sm text-slate-500 text-center mt-3 px-8 leading-relaxed">
        ...
        <button className="mt-8 text-indigo-400 text-xs font-black uppercase tracking-widest bg-indigo-500/10 px-6 py-3 rounded-2xl border border-indigo-500/20 hover:bg-indigo-500 hover:text-white transition-all pointer-events-none">
          Browse Files
        </button>
          ...
          {[...uploading, ...uploadedFiles].map((file, i) => (
            <div key={i} className="flex items-center justify-between p-4 bg-slate-800/40 rounded-2xl border border-slate-800 hover:bg-slate-800 transition-colors group">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <span className="text-sm text-slate-300 font-bold block">{file.name}</span>
...
const StudyRoomsPage = () => {
  ...
  return (
    <div className="w-full flex gap-6 h-[calc(100vh-10rem)] max-h-[700px]">
      {/* Room list */}
      <div className="w-64 shrink-0 bg-slate-900 border border-slate-800 rounded-[2rem] overflow-hidden flex flex-col">
        <div className="p-4 border-b border-slate-800 bg-slate-800/30">
          <h3 className="font-black text-white text-sm">📚 Study Rooms</h3>
          <div className="flex items-center gap-2 mt-1">
            <div className={`w-2 h-2 rounded-full ${connected ? 'bg-emerald-400' : 'bg-red-400'}`} />
            <p className="text-[10px] text-slate-500 font-bold">{connected ? 'Connected' : 'Connecting…'}</p>
          </div>
        </div>
          ...
          {SYLLABUS.map(sub => (
            <button key={sub.code} onClick={() => joinRoom(sub)}
              className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all text-left
                ${activeRoom === sub.code ? 'bg-indigo-600 text-white' : 'text-slate-400 hover:bg-slate-800 hover:text-white'}`}>
      ...
      {/* Chat area */}
      <div className="flex-1 bg-slate-900 border border-slate-800 rounded-[2rem] overflow-hidden flex flex-col">
        {activeRoom ? (
          <>
            {/* Chat header */}
            <div className="p-4 border-b border-slate-800 flex items-center justify-between bg-slate-800/30">
              <div>
                <div className="flex items-center gap-2">
                  ...
                  <h4 className="font-black text-white text-sm">{activeSubject?.name}</h4>
                </div>
                <p className="text-[10px] text-slate-500 mt-0.5">{activeSubject?.description}</p>
              </div>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4">
              ...
              {messages.map((msg) => (
                <div key={msg.id} className="flex gap-3 group">
                  <div className="w-9 h-9 rounded-full bg-gradient-to-br from-indigo-600 to-cyan-500 flex items-center justify-center text-white text-[11px] font-black shrink-0">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-slate-300 mt-0.5 leading-relaxed">{msg.message}</p>
                  </div>
                </div>
              ))}
              <div ref={endRef} />
            ...
            {/* Input */}
            <div className="p-3 border-t border-slate-800 flex gap-3">
              <input value={input} onChange={(e) => setInput(e.target.value)}
                onKeyDown={(e) => e.key === 'Enter' && !e.shiftKey && sendMessage()}
                placeholder={`Message #${activeSubject?.code}…`}
                className="flex-1 bg-slate-800 rounded-xl px-4 py-3 text-sm outline-none focus:ring-1 focus:ring-indigo-500 transition-all placeholder:text-slate-600" />
          ...
          <div className="flex-1 flex flex-col items-center justify-center gap-6 text-slate-600">
            <Hash size={48} />
            <div className="text-center">
              <p className="text-white font-bold text-lg mb-2">Welcome to Study Rooms</p>
...
const DashboardShell = () => {
  ...
  return (
    <div className={`min-h-screen ${isDark ? 'bg-slate-950 text-slate-200' : 'bg-slate-100 text-slate-800'} flex overflow-hidden font-sans antialiased transition-colors duration-300`}>
      {sidebarOpen && <div className="fixed inset-0 bg-black/60 z-30 lg:hidden" onClick={() => setSidebarOpen(false)} />}
      <Sidebar isOpen={sidebarOpen} onClose={() => setSidebarOpen(false)} />
      <main className="flex-1 overflow-y-auto relative flex flex-col">
        <header className={`h-16 lg:h-20 border-b ${isDark ? 'border-slate-800/60 bg-slate-950/40' : 'border-slate-200 bg-white/80'} flex items-center justify-between px-4 lg:px-10 backdrop-blur-xl sticky top-0 z-30 transition-colors duration-300`}>
          <div className="flex items-center gap-4">
            <button onClick={() => setSidebarOpen(true)} className="p-2 hover:bg-slate-800/40 rounded-xl text-slate-400 transition-colors lg:hidden">
              <Menu size={20} />
            </button>
            <div>
              <h2 className="text-base lg:text-lg font-bold tracking-tight">{pageTitle}</h2>
              <p className="text-[9px] lg:text-[10px] text-slate-500 uppercase tracking-widest font-black hidden sm:block">{uid} · {name}</p>
            </div>
          </div>
          <div className="flex items-center gap-3 lg:gap-5">
            {/* ✅ Dark/Light Mode Toggle */}
            <button onClick={toggleTheme}
              className={`p-2 lg:p-2.5 ${isDark ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'} border rounded-full transition-all hover:scale-110`}
              title={isDark ? 'Switch to Light Mode' : 'Switch to Dark Mode'}>
              {isDark ? <Sun size={16} className="text-amber-400" /> : <Moon size={16} className="text-indigo-600" />}
            </button>

            <button className={`p-2 lg:p-2.5 ${isDark ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'} border rounded-full text-slate-400 relative hover:text-white transition-all`}>
            ...
            <div className="relative" ref={dropRef}>
              <button onClick={() => setShowProfileDrop(p => !p)}
                ...
                <div className={`w-full h-full rounded-full ${isDark ? 'bg-slate-900' : 'bg-white'} flex items-center justify-center font-black text-[10px] lg:text-xs text-white`}>
                  {initials}
                </div>
              </button>
              {showProfileDrop && (
                <div className={`absolute right-0 mt-3 w-56 ${isDark ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'} border rounded-2xl shadow-2xl overflow-hidden`}>
                  ...
                  <div className="p-2">
                    <button className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl ${isDark ? 'hover:bg-slate-800' : 'hover:bg-slate-100'} transition-all text-sm font-medium`}>
